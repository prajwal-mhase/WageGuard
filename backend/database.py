...
_SEED_RATES = [
    ("Maharashtra", "Construction of Roads and Buildings", "Zone I", "unskilled",
     ...
     "Daily figure = monthly total / 26 working days. MVP conversion, not an officially published daily rate."),
    ...
    try:
        conn.executescript("""
            ...
            create table if not exists wage_rates (
                ...
                state text not null,
                scheduled_employment text not null,
                zone text not null,
                skill_tier text not null,
                monthly_total real not null,
                daily_equivalent real not null,
            ...
            create table if not exists job_entries (
                ...
                normalized_daily_equivalent real not null,
                potential_gap real not null,
                status text not null,
                calculation_note text not null,
                created_at text not null default current_timestamp
            );
            create index if not exists idx_job_entries_user_date
                on job_entries(user_id, job_date desc);
        """)
        # executescript auto-commits; check and seed separately
        count = conn.execute("select count(*) from wage_rates").fetchone()[0]
        if count == 0:
            conn.executemany(
